...
class LSegModule(LSegmentationModule):
    def __init__(self, data_path, dataset, batch_size, base_lr, max_epochs, **kwargs):
        ...
        labels = self.get_labels('replica')
    ...
    def get_labels(self, dataset):
        ...
        if dataset == 'replica':
            # For replica dataset, use simple .txt file with one label per line
            path = 'Lseg/label_files/{}.txt'.format(dataset)
            assert os.path.exists(path), '*** Error : {} not exist !!!'.format(path)
            f = open(path, 'r') 
            lines = f.readlines()      
            for line in lines: 
                label = line.strip()  # Simple format: one label per line
                if label:  # Skip empty lines
                    labels.append(label)
            f.close()
        else:
            # For ade20k and other datasets, use original format
            ...
            if dataset in ['ade20k']:
                labels = labels[1:]  # Remove header line for ade20k
