...
def undistort_feature_map_torch(feature_map, map1x, map1y):
    ...
    Args:
        feature_map: (C, H, W) torch tensor on CUDA
        map1x, map1y: np.array of shape (H, W) from cv2.initUndistortRectifyMap
    Returns:
        undistorted feature_map (C, H, W)
    """
...
def main(args):
    ...
    pt_files = [f for f in files_all if f.endswith(".pt")]
    ...
    for pt_file in tqdm(pt_files):
        ...
        torch.save(semantic_feature_undistorted.half().cpu(), os.path.join(args.output_path, pt_file))
    
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Undistort feature maps (.pt files) using OpenCV maps and torch grid_sample.")
    parser.add_argument("--input_path", type=str, required=True, help="Path to input directory containing .pt files")
    parser.add_argument("--output_path", type=str, required=True, help="Path to save undistorted .pt files")
