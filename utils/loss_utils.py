...
def cos_loss(network_output, gt):
    network_output_norm = F.normalize(network_output, p=2, dim=-1)
    gt_norm = F.normalize(gt, p=2, dim=-1)
    cos_sim = torch.sum(network_output_norm * gt_norm, dim=-1, keepdim=True)
    loss = 1.0 - cos_sim
    loss = torch.where(gt.sum(dim=-1, keepdim=True)!=0, loss, 0.)
