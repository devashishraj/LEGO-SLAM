...
# Global variables for PCA caching
pca_mean = None
top_vector = None
...
def feature_map(feature):
    """Convert high-dimensional feature map to RGB using PCA"""
    global pca_mean
    global top_vector
...
def gradient_map(image):
    """Calculate gradient/edge map from RGB image"""
...
def colormap(map, cmap="turbo"):
    """Apply colormap to single channel tensor"""
...
def render_net_image(render_pkg, render_items, render_mode, camera):
    """Generate network image based on render mode"""
    output = render_items[render_mode].lower()
    
    # Handle LEGO SLAM render package format
    depth_key = "render_depth" if "render_depth" in render_pkg else "depth"
    feature_key = "semantic_feature" if "semantic_feature" in render_pkg else "feature_map"
    
    if output == 'depth':
        net_image = render_pkg[depth_key]
    elif output == 'edge':
        net_image = gradient_map(render_pkg["render"])
    elif output == 'normal':
        net_image = depth_to_normal(render_pkg[depth_key], camera).permute(2,0,1)
        ...
        if feature_key in render_pkg:
            net_image = feature_map(render_pkg[feature_key])
        else:
            print(f"Warning: No feature map available, showing RGB instead")
