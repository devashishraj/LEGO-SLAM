...
PYBIND11_MODULE(pygicp, m) {
  ...
  py::class_<LsqRegistration, std::shared_ptr<LsqRegistration>>(m, "LsqRegistration")
    ...
    .def("set_max_iteration", &LsqRegistration::setMaxIteration)
    .def("set_min_iteration", &LsqRegistration::setMinIteration)
    ...
    .def("is_converged", &LsqRegistration::isConverged)
    .def("align",
  ...
  py::class_<FastGICP, LsqRegistration, std::shared_ptr<FastGICP>>(m, "FastGICP")
    ...
    .def("set_input_source_sparse", [] (FastGICP& gicp, const Eigen::Matrix<double, -1, 3>& points) { gicp.setInputSourceSparse(eigen2pcl(points)); })
    ...
    .def("set_edge_weight", &FastGICP::setEdgeWeight)
    .def("set_sparse_weight", &FastGICP::setSparseWeight)
    ...
    .def("get_source_edge_mask", [] (FastGICP& gicp){
      return py::array(gicp.getSourceEdgeMaskSize(), gicp.getSourceEdgeMask().data());
    })
    .def("get_target_edge_mask", [] (FastGICP& gicp){
      return py::array(gicp.getTargetEdgeMaskSize(), gicp.getTargetEdgeMask().data());
    })
    ...
    .def("set_target_covariances_fromqs_additional", [] (FastGICP& gicp, py::array rotationsq, py::array scales, py::array edge_mask){
    	if(py::len(rotationsq)/4!=py::len(scales)/3){ std::cerr<<"qs size not matched" <<std::endl; return;}
    	const auto input_rotationsq = rotationsq.cast<std::vector<float>>();
    	const auto input_scales = scales.cast<std::vector<float>>();
      const auto input_edge_mask = edge_mask.cast<std::vector<int>>();
    	gicp.setTargetCovariancesAdditional(input_rotationsq, input_scales, input_edge_mask);
    })
    ...
    .def("set_source_edge_invalid_mask", [] (FastGICP& gicp, py::array mask){
      const auto input_filter = mask.cast<std::vector<int>>();
    	gicp.setSourceEdgeInvalidMask(input_filter);
    })
    .def("set_target_edge_invalid_mask", [] (FastGICP& gicp, py::array mask){
      const auto input_filter = mask.cast<std::vector<int>>();
    	gicp.setTargetEdgeInvalidMask(input_filter);
    })
    .def("get_source_knn_idxs", [] (FastGICP& gicp) {
      return py::array(gicp.getSourceKnnIdxsSize(), gicp.getSourceKnnIdxs().data());
    })
  ;
