...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    rendered_image, feature_map, radii, depth_image, out_opacity = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
...
def render_3(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
    ...
    # Rasterize visible Gaussians to image, obtain their radii (on screen). 
    # depth_image, rendered_image, radii, is_used = rasterizer(
    rendered_image, feature_map, radii, depth_image, alpha_image = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
        colors_precomp = colors_precomp,
...
def render_3_filtered(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
           training_stage=0):
    """
    Render the scene. 
    ...
    opacity_filter = pc.get_opacity > 0.8
    opacity_filter = opacity_filter.squeeze(1)
 
    # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means
    screenspace_points = torch.zeros_like(pc.get_xyz[opacity_filter], dtype=pc.get_xyz.dtype, requires_grad=True, device="cuda") + 0
    try:
        screenspace_points.retain_grad()
    except:
        pass
    ...
    means3D = pc.get_xyz[opacity_filter]
    means2D = screenspace_points
    opacity = pc.get_opacity[opacity_filter]
    ...
    cov3D_precomp = None
    if pipe.compute_cov3D_python:
        cov3D_precomp = pc.get_covariance(scaling_modifier)[opacity_filter]
    else:
        scales = pc.get_scaling[opacity_filter]
        rotations = pc.get_rotation[opacity_filter]
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            ...
            shs = pc.get_features[opacity_filter]
    else:
        colors_precomp = override_color
    
    semantic_feature = pc.get_semantic_feature[opacity_filter]
